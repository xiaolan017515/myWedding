...
//define("TOKEN", "zhengfeng");
class FangdaiController extends WeixinController{
    private $appid;
    private $secret;
    ...
    */
    public function getToken(){
        if(!is_dir(FANGDAI_TOKEN)){
            mkdir(FANGDAI_TOKEN,0755,true);
        }
        $url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={$this->appid}&secret={$this->secret}";
        $token = $this->readToken(FANGDAI_TOKEN);
        $time = time();
        if(!is_null($token)){
            if($token['time'] < $time || is_null($token['access_token'])){
                ...
                $this->writeToken(FANGDAI_TOKEN,$this->access_token,$time);
            }else{
    ...
    public function getJsapi(){
        ...
        if(!is_null($token)){
            if($token['time'] < $time || is_null($token['access_token'])){
                ...
                $data = json_decode($data,true);
                $ticket = $data['ticket'];
                $this->writeToken(FANGDAI_JSAPI,$ticket,$time);
            }else{
    ...
    */
    public function createQrcode($scene_id = '99999'){
        ...
        if(!is_null($scene_id)){
            ...
            if(!is_dir(FANGDAI_QRCODE)){
                mkdir(FANGDAI_QRCODE,0755,true);
            }
            
            $file_name = FANGDAI_QRCODE."/{$scene_id}.jpg";
    ...
    */
    ...
    public function receiveCode(){
        ...
        if(!is_null($code) &&  !is_null($state)){
            ...
            $url = isset($url['url']) ? FANGDAI_PATH.$url['url'] : NULL;
            if(!is_null($url)){
               header("location:$url");
               exit();
            }else{
