...
class TestController extends WeixinController{
    ...
    private  $dir;//各种文件的前缀
    private $source;
    private $EncodingAESKey;//消息加解密密钥
    private $wxBiz;//微信消息加解密

    public function __construct(){

        $this->token = 'test';
        
        $this->appid = 'wx61e21f1e24525cb3';
        
        $this->secret = '419aadbf86325a93f1d87da2d20bf31c';
        
        $this->EncodingAESKey = 'iPzhxkBiOrEbUZYsDD7XGz85h46nLbKqG5fqaGCCnLt';
        ...
        $this->dir = __DIR__.'/../Public/test';
        
        if(!is_dir($this->dir)){
            mkdir($this->dir,0755,true);
        }
        
        $this->wxBiz = new WXBizMsgCrypt($this->token,$this->EncodingAESKey,$this->appid);
    ...
    public function responseMag(){
        if($this->checkSignature($this->token)){
            ...
            if(!empty($post_str)){             
                ...
                $nonce = $_GET["nonce"];        
                $mmsg = '';
                $errCode = $this->wxBiz->decryptMsg($msg_signature, $timestamp, $nonce, $post_str, $mmsg);

                //解密后的xml
                
                $post_str = $post_str = simplexml_load_string($mmsg, 'SimpleXMLElement', LIBXML_NOCDATA); ;
    ...
    */
    ...
    public function sendNew($msg,$nonce){
        $base_info = $this->getInfo($this->access_token,$msg['from']);
        $data = array(
            '0'=>array(
        ...
        $timestamp = time();
        $str =  $this->imgTextTel($msg['from'],$msg['to'],$timestamp,count($data),$data);
        $encryptMs = '';
        $this->wxBiz->encryptMsg($str,$timestamp,$nonce,$encryptMsg);
        echo $encryptMsg;
    }
